CREATE TABLE `keyword_universe_runs` (
	`id` text PRIMARY KEY NOT NULL,
	`project_id` text NOT NULL,
	`sources_json` text NOT NULL,
	`status` text DEFAULT 'pending' NOT NULL,
	`seen_count` integer,
	`kept_count` integer,
	`error` text,
	`started_at` text DEFAULT (current_timestamp) NOT NULL,
	`finished_at` text,
	FOREIGN KEY (`project_id`) REFERENCES `projects`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `keyword_universe_runs_one_active_per_project_idx` ON `keyword_universe_runs` (`project_id`) WHERE "keyword_universe_runs"."status" IN ('pending', 'running');--> statement-breakpoint
CREATE TABLE `project_gate` (
	`id` text PRIMARY KEY NOT NULL,
	`project_id` text NOT NULL,
	`positives_json` text NOT NULL,
	`negatives_json` text NOT NULL,
	`brand_tokens_json` text NOT NULL,
	`kd_ceiling` integer DEFAULT 20 NOT NULL,
	`kd_ceiling_updated_at` text,
	`user_edited` integer DEFAULT false NOT NULL,
	`derived_at` text DEFAULT (current_timestamp) NOT NULL,
	`updated_at` text DEFAULT (current_timestamp) NOT NULL,
	FOREIGN KEY (`project_id`) REFERENCES `projects`(`id`) ON UPDATE no action ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `project_gate_project_idx` ON `project_gate` (`project_id`);