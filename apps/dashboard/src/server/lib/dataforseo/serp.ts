...
export async function fetchLiveSerp(input: {
  ...
  locationCode: number;
  languageCode: string;
  /** Results to crawl, in pages of 10. Defaults to the full 100 the keyword
   *  research screens read; callers that only need page one (the page plan's
   *  verdict is decided entirely inside the top 10) pass 10 and pay for one
   *  page instead of ten. */
  depth?: number;
